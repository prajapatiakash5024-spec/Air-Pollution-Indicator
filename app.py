...
[data-testid="stSidebar"] {
    background: linear-gradient(180deg, #060f1f 0%, #091525 100%) !important;
...
[data-testid="stSidebar"] .stButton > button {
    background: linear-gradient(135deg, #0080ff22, #00f5ff22) !important;
...
[data-testid="stSidebar"] .stButton > button:hover {
    background: linear-gradient(135deg, #0080ff44, #00f5ff44) !important;
...
if "live_aqi" not in st.session_state:
    st.session_state.live_aqi      = 142
if "live_city" not in st.session_state:
    st.session_state.live_city     = "Mumbai"
if "live_history" not in st.session_state:
    st.session_state.live_history  = [142]
...
with st.sidebar:
    ...
    st.session_state.auto_refresh = st.toggle("⚡ Auto-Refresh (30s)", value=st.session_state.auto_refresh)
    ...
    for lo, hi, label, color, emoji, _ in AQI_SCALE:
        st.markdown(
            ...
            f'font-family:\'Exo 2\';font-size:0.78rem;">'
...
with col_title:
    ...
    <p style="font-family:'Share Tech Mono',monospace;color:#6a8fad;font-size:0.82rem;
              letter-spacing:1.5px;margin-top:0;">
...
if "Live" in view_mode:
    ...
    with lcol2:
        ...
        if st.button("⚡  SIMULATE NEW READING", use_container_width=True):
            ...
            new_aqi = max(10, min(500, prev + delta))
    ...
    with gauge_col:
        ...
        fig_gauge = go.Figure(go.Indicator(
            ...
            number={"font": {"size": 56, "family": "Orbitron, sans-serif", "color": color},
                    "suffix": " AQI"},
            gauge={
                "axis": {
                    "range": [0, 500],
            ...
            title={"text": f"{emoji} {label}<br><span style='font-size:11px;color:#6a8fad;font-family:Exo 2;'>{live_city} · Live Sensor</span>",
        ...
        <div class="health-advice" style="border-color:{color}33;">
            <div style="font-family:'Orbitron',sans-serif;font-size:0.75rem;
                        color:{color};letter-spacing:2px;margin-bottom:8px;">
                HEALTH ADVISORY
            </div>
    ...
    with detail_col:
        ...
        for p in POLLUTANTS:
            ...
            <div style="margin:6px 0;padding:10px 14px;
                ...
                <div style="display:flex;justify-content:space-between;
                    ...
                    <span style="font-family:'Orbitron',sans-serif;font-size:0.8rem;
                                 color:#e0f4ff;font-weight:700;">{p}</span>
                    <span style="font-family:'Share Tech Mono',monospace;font-size:0.85rem;
                                 color:{s_color};">{val} {UNITS[p]}</span>
                ...
                <div style="background:rgba(0,0,0,0.3);border-radius:4px;height:6px;overflow:hidden;">
                    <div style="width:{bar_w}%;height:100%;
                                background:linear-gradient(90deg, {s_color}88, {s_color});
                                border-radius:4px;transition:width 0.5s ease;"></div>
                </div>
                <div style="font-family:'Share Tech Mono';font-size:0.68rem;
                            color:#6a8fad;margin-top:4px;">
        ...
        for tip in tips:
            ...
            <div style="padding:6px 12px;margin:4px 0;
                        background:rgba(0,245,255,0.04);border-radius:6px;
                        font-family:'Exo 2';font-size:0.85rem;color:#e0f4ff;">
                {tip}
            </div>
            """, unsafe_allow_html=True)
    ...
    fig_stream.add_trace(go.Scatter(
        ...
        name="AQI",
        hovertemplate="Reading %{x}: AQI %{y}<extra></extra>",
    ))
    ...
    for threshold, t_label, t_color in [(100, "Satisfactory", "#a3ff00"), (200, "Moderate", "#ffb800"), (300, "Poor", "#ff6600")]:
        fig_stream.add_hline(y=threshold, line_dash="dash", line_color=t_color, opacity=0.4,
                             annotation_text=t_label, annotation_font_size=10,
                             annotation_font_color=t_color)

    # Current marker
    if hist:
        fig_stream.add_trace(go.Scatter(
            x=[len(hist)-1], y=[hist[-1]],
            mode="markers",
            marker=dict(size=16, color=color, symbol="circle",
                        line=dict(width=2, color="#ffffff"),
                        opacity=0.9),
            name="Current",
            hovertemplate=f"CURRENT: AQI {hist[-1]}<extra></extra>",
        ))
    ...
    # ── Air Quality Forecast (simulated 24h) ──────────────────────────────
    st.markdown('<div class="section-header">🔮 24-HOUR AQI FORECAST</div>', unsafe_allow_html=True)

    now_h    = datetime.datetime.now().hour
    hours    = [(now_h + i) % 24 for i in range(24)]
    h_labels = [f"{h:02d}:00" for h in hours]
    forecast = [
        max(20, min(500, int(current_aqi + 40 * np.sin((i - 6) * np.pi / 12) + random.randint(-20, 20))))
        for i in range(24)
    ]
    ...
    fig_fc.update_layout(
        **PLOT_TEMPLATE,
        ...
        xaxis_title="Hour",
        yaxis_title="AQI",
        xaxis=dict(tickangle=-45, **PLOT_TEMPLATE["xaxis"]),
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with col_left:
        ...
        map_style  = mapc2.selectbox("Map theme",
        ...
        dff2["label"]    = dff2.apply(
            lambda r: f"<b>{r['City']}</b>  {r['Emoji']}<br>"
                      f"AQI: {r['AQI']}  ·  {r['Category']}<br>"
        ...
        fig_map = go.Figure(go.Scattermapbox(
            ...
            marker=dict(
                ...
                color=dff2["AQI"],
                colorscale=[
                    [0.00, "#00ff88"], [0.10, "#00ff88"],
                    [0.10, "#a3ff00"], [0.20, "#a3ff00"],
                    [0.20, "#ffb800"], [0.40, "#ffb800"],
                    [0.40, "#ff6600"], [0.60, "#ff6600"],
                    [0.60, "#ff3366"], [0.80, "#ff3366"],
                    [0.80, "#b44dff"], [1.00, "#b44dff"],
                ],
                cmin=0, cmax=500,
                colorbar=dict(title="AQI", thickness=12, len=0.6,
                              tickfont=dict(color="#e0f4ff"),
                              titlefont=dict(color="#e0f4ff")),
                opacity=0.9,
            ),
    ...
    with col_right:
        ...
        for _, row in ranked.iterrows():
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;
                        ...
                        background:rgba(0,245,255,0.04);border-left:3px solid {row['Color']};">
                <span style="font-family:'Exo 2';font-size:0.82rem;font-weight:600;
                             color:#e0f4ff;">{row['Emoji']} {row['City']}</span>
                <span style="font-family:'Share Tech Mono';font-size:0.8rem;
                             color:{row['Color']};">{row['AQI']}</span>
            </div>
            """, unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    fig_bar.update_layout(
        **PLOT_TEMPLATE,
        ...
        yaxis=dict(title=f"{sort_by} ({unit})" if unit else sort_by, **PLOT_TEMPLATE["yaxis"]),
        height=440,
    )
    ...
    with scatter_col:
        ...
        for cat, grp in df.groupby("Category"):
            ...
            fig_sc.add_trace(go.Scatter(
                x=grp["PM2.5"], y=grp["PM10"],
                mode="markers+text",
                name=cat,
                text=grp["City"],
                textposition="top center",
                textfont=dict(size=8, color="#6a8fad"),
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if not selected_cities:
        ...
        for i, city in enumerate(selected_cities[:6]):
            ...
            trend = [
                max(20, min(500, int(base + 55 * np.sin((h - 6) * np.pi / 12) + random.randint(-25, 25))))
                for h in hours
            ]
        ...
        fig_line.update_layout(
            **PLOT_TEMPLATE,
            ...
            legend=dict(orientation="h", yanchor="bottom", y=1.02,
                        bgcolor="rgba(10,22,40,0.8)", bordercolor="rgba(0,245,255,0.2)", borderwidth=1),
            height=450,
        )
        st.plotly_chart(fig_line, use_container_width=True)
        st.info("💡 Trend uses a sine-wave diurnal pattern (peaks ~6 AM and 6 PM) with random noise. Connect CPCB/WAQI API for live data.")
...
# ══════════════════════════════════════════════════════════════════════════════
        ...
        fig_radar.update_layout(
            polar=dict(
                radialaxis=dict(visible=True, range=[0, 100],
                                tickfont=dict(color="#6a8fad", size=9),
    ...
    bar_colors = [
        "#00ff88" if v <= safe_val else "#ffb800" if v <= safe_val * 1.5 else "#ff3366"
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with col1:
        ...
        for i, r in best10.iterrows():
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;
                        ...
                        background:rgba(0,255,136,0.06);border-left:3px solid {r['Color']};">
                <span style="font-family:'Exo 2';font-size:0.9rem;">
                    <b>{i+1}.</b> {r['Emoji']} <b>{r['City']}</b>
                    <span style="color:#6a8fad;font-size:0.78rem;"> · {r['State']}</span>
                </span>
                <span style="font-family:'Share Tech Mono';color:{r['Color']};font-size:0.9rem;">
                    {r['AQI']}
                </span>
            </div>
            """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        for i, r in worst10.iterrows():
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;
                        padding:8px 12px;margin:4px 0;border-radius:8px;
                        background:rgba(255,51,102,0.06);border-left:3px solid {r['Color']};">
                <span style="font-family:'Exo 2';font-size:0.9rem;">
                    <b>{i+1}.</b> {r['Emoji']} <b>{r['City']}</b>
                    <span style="color:#6a8fad;font-size:0.78rem;"> · {r['State']}</span>
                </span>
    ...
    fig_state.update_layout(
        **PLOT_TEMPLATE,
        ...
        xaxis=dict(tickangle=-35, **PLOT_TEMPLATE["xaxis"]),
        height=380,
    )
    ...
    with corr_col:
        ...
        fig_heat = go.Figure(go.Heatmap(
            ...
            colorscale=[[0, "#ff3366"], [0.5, "#0a1628"], [1, "#00ff88"]],
            zmin=-1, zmax=1,
        ...
        for lo, hi, label_v, color_v, emoji_v, _ in AQI_SCALE:
            ...
            if len(subset) > 0:
                fig_v.add_trace(go.Violin(
                    y=subset, name=f"{emoji_v} {label_v}",
                    fillcolor=f"rgba({int(color_v[1:3],16)},{int(color_v[3:5],16)},{int(color_v[5:7],16)},0.3)",
                    line_color=color_v,
...
# ══════════════════════════════════════════════════════════════════════════════
# VIEW 6 ─ WEATHER & AQI FORECAST  (NEW)
# ══════════════════════════════════════════════════════════════════════════════
    ...
    fig_fc = make_subplots(
        ...
        subplot_titles=["AQI Forecast", "Temperature (°C)", "Humidity (%)"],
        vertical_spacing=0.08,
    )
    fc_colors = [get_aqi_info(v)[1] for v in aqi_fc]
    fig_fc.add_trace(go.Bar(x=d_labels, y=aqi_fc, marker_color=fc_colors,
                            name="AQI", hovertemplate="%{x}: AQI %{y}<extra></extra>"), row=1, col=1)
    fig_fc.add_trace(go.Scatter(x=d_labels, y=temp_fc, mode="lines+markers",
                                line=dict(color="#ff6600", width=2.5),
                                marker=dict(size=7, color="#ff6600"),
                                name="Temp °C", hovertemplate="%{x}: %{y}°C<extra></extra>"), row=2, col=1)
    fig_fc.add_trace(go.Bar(x=d_labels, y=humid_fc,
    ...
    fig_fc.update_layout(
        **PLOT_TEMPLATE,
        ...
        title=dict(text=f"7-Day Forecast · {fcity}", font=dict(family="Orbitron", color="#00f5ff", size=14)),
    )
    for i in range(1, 4):
        fig_fc.update_xaxes(gridcolor="rgba(0,245,255,0.08)", linecolor="rgba(0,245,255,0.2)", row=i, col=1)
        fig_fc.update_yaxes(gridcolor="rgba(0,245,255,0.08)", linecolor="rgba(0,245,255,0.2)", row=i, col=1)
    st.plotly_chart(fig_fc, use_container_width=True)
    ...
    st.dataframe(
        forecast_df.style.background_gradient(subset=["AQI"], cmap="RdYlGn_r"),
    ...
    with result_col:
        if uploaded_file:
            if st.button("🔍  ANALYZE IMAGE", use_container_width=True):
                ...
                <div class="aqi-live-card" style="border-color:{p_color}44;">
                    <div style="font-family:'Share Tech Mono';font-size:0.72rem;
                                color:#6a8fad;letter-spacing:2px;margin-bottom:8px;">
                        VISUAL HAZE ANALYSIS RESULT
                    </div>
                    <div class="aqi-number" style="color:{p_color};
                                text-shadow:0 0 30px {p_color}80;">
                        {pollution}%
                    </div>
                    <div class="aqi-label-text" style="color:{p_color};">
                        {p_emoji} {p_label}
                    </div>
                    <div style="font-family:'Exo 2';font-size:0.82rem;
                                color:#6a8fad;margin-top:8px;">
                ...
                fig_img_gauge = go.Figure(go.Indicator(
                    ...
                    gauge={
                        "axis": {"range": [0, 100], "tickfont": {"color": "#6a8fad", "size": 9}},
                        "bar": {"color": p_color},
                        "bgcolor": "rgba(0,0,0,0)",
                        "steps": [
                            {"range": [0, 20],  "color": "rgba(0,255,136,0.1)"},
                ...
                # Health advice
                st.markdown(f"""
                <div class="health-advice" style="border-color:{p_color}44;margin-top:10px;">
                    <b style="color:{p_color};">Advisory:</b> {p_advice}
                </div>
                """, unsafe_allow_html=True)

                # AQI scale table
                st.divider()
                st.markdown('<div class="section-header">📊 VISUAL POLLUTION SCALE</div>', unsafe_allow_html=True)
                for lo, hi, lbl, clr, emj, _ in AQI_SCALE:
                    plo, phi = lo // 5, hi // 5
                    st.markdown(f"""
                    <div style="display:flex;align-items:center;gap:10px;padding:5px 10px;
                                margin:3px 0;border-radius:6px;
                                background:{'rgba('+str(int(clr[1:3],16))+','+str(int(clr[3:5],16))+','+str(int(clr[5:7],16))+',0.12)' if pollution >= plo else 'rgba(0,0,0,0)'};
                                border-left:3px solid {clr};">
                        <span>{emj}</span>
                        <span style="font-family:'Exo 2';font-size:0.82rem;
                                     color:#e0f4ff;font-weight:600;">{lbl}</span>
                        <span style="font-family:'Share Tech Mono';font-size:0.75rem;
                                     color:#6a8fad;">{plo}% – {phi}%</span>
                    </div>
                    """, unsafe_allow_html=True)
        else:
            st.markdown("""
            <div style="text-align:center;padding:60px 20px;
                        ...
                        border-radius:12px;font-family:'Exo 2';">
                <div style="font-size:3rem;margin-bottom:12px;">📷</div>
                <div style="color:#6a8fad;font-size:0.9rem;">
                    Upload a sky or outdoor image<br>to begin visual haze analysis
                </div>
            </div>
            """, unsafe_allow_html=True)

# ══════════════════════════════════════════════════════════════════════════════
# VIEW 8 ─ DATA EXPORT  (NEW)
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with exp_col1:
        ...
        csv_data = df[["City", "State", "AQI", "Category"] + POLLUTANTS].to_csv(index=False)
        st.download_button(
            label="⬇️  Download Full Dataset (CSV)",
            data=csv_data,
            file_name=f"india_aqi_{datetime.date.today()}.csv",
            mime="text/csv",
            use_container_width=True,
        )
        st.markdown('<div style="height:8px;"></div>', unsafe_allow_html=True)

        top10_csv = df.nlargest(10, "AQI")[["City", "State", "AQI", "Category"] + POLLUTANTS].to_csv(index=False)
        st.download_button(
            label="⬇️  Download Top 10 Polluted Cities (CSV)",
            data=top10_csv,
            file_name=f"top10_polluted_{datetime.date.today()}.csv",
            mime="text/csv",
            use_container_width=True,
        )
        st.markdown('<div style="height:8px;"></div>', unsafe_allow_html=True)

        safe_csv = df[df["AQI"] <= 100][["City", "State", "AQI", "Category"] + POLLUTANTS].to_csv(index=False)
        st.download_button(
            label="⬇️  Download Safe Cities Report (CSV)",
            data=safe_csv,
            file_name=f"safe_cities_{datetime.date.today()}.csv",
            mime="text/csv",
            use_container_width=True,
        )
...
st.markdown("""
<div style="display:flex;justify-content:space-between;align-items:center;
            font-family:'Share Tech Mono',monospace;font-size:0.72rem;color:#6a8fad;
            padding:8px 0;">
